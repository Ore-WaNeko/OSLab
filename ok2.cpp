...
void checkCompleteTimer(queues q[], int current_queue){
    bool a = notComplete(q);
    // Only increment total_time for the currently executing queue
    if(q[current_queue].executed==false){
        for(int j=0;j<q[current_queue].length;j++){
            if(q[current_queue].p[j].burst_time!=0){
                q[current_queue].p[j].total_time+=1;
            }
        }
        q[current_queue].total_time+=1;
    }
}
